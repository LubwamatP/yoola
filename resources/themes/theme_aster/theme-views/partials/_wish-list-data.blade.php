                        ...
                        <td>
                            <div class="media gap-3 align-items-center mn-w200">
                                <div class="avatar border rounded size-3-437rem">
                                    <img class="img-fit dark-support rounded border aspect-1" alt="{{ $product['name'] ?? 'Product' }}"
                            ...
                            <div class="d-flex justify-content-center gap-2 align-items-center">
                                <a href="javascript:"
                                   ...
                                   id="compare_list-{{$product['id']}}">
                                    <img src="{{ theme_asset('assets/img/icons/compare.svg') }}" class="svg" alt="{{ $product['name'] ?? 'Product' }}">
                                </a>
                                <a href="#" data-action="{{ route('cart.add') }}" data-id="{{$product['id']}}" data-quantity="{{$product['minimum_order_qty'] ?? 1}}" data-key="" data-product_variation_code="{{$wishlist['product_id']}}"
                                   ...
                                   id="">
                                    <img src="{{ theme_asset('assets/img/icons/cart.svg') }}" class="svg" alt="{{ $product['name'] ?? 'Product' }}">
                                </a>
                            </div>
                        </td>
                        <td>
...
@if($wishlists->count()==0)
    <div class="d-flex flex-column justify-content-center align-items-center gap-2 py-3 w-100">
        <img width="80" class="mb-3" src="{{ theme_asset('assets/img/empty-state/empty-wishlist.svg') }}" alt="{{ $product['name'] ?? 'Product' }}">
            ...
            @if( $wishlist->productFullInfo)
                <div class="media gap-3 bg-light p-3 rounded">
                    <div class="avatar border rounded size-3-437rem">
                        <img
                            src="{{ getStorageImages(path:$product->thumbnail_full_url, type: 'product') }}"
                            class="img-fit dark-support rounded" alt="{{ $product['name'] ?? 'Product' }}">
                    </div>
                        ...
                        <div class="modal-body px-20 py-0 mb-30">
                            <form action="{{ route('delete-wishlist') }}" method="post" class="remove-wishlist-form">
                                ...
                                <div class="d-flex flex-column align-items-center text-center mb-30">
                                    <img src="{{ theme_asset('assets/img/modal/wishlist-clear.svg') }}" width="70" class="aspect-1 h-70px mb-20" id="custom-modal-1753511225620-image" alt="{{ $product['name'] ?? 'Product' }}">
