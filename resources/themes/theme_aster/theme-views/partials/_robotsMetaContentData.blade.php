...
{{-- ====== YOOLA CANONICAL URL TAG ====== --}}
@php
    // Build canonical URL - always use https://yoola.ug (no www, no mail)
    $canonicalUrl = 'https://yoola.ug' . request()->getPathInfo();
    
    // Remove trailing slash except for homepage
    if ($canonicalUrl !== 'https://yoola.ug/' && str_ends_with($canonicalUrl, '/')) {
        $canonicalUrl = rtrim($canonicalUrl, '/');
    }
    
    // Remove query parameters for clean canonical (except essential ones)
    // Products with pagination should canonicalize to page 1
@endphp
