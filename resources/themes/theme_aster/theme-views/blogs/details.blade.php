                ...
                <div class="row g-4 justify-content-center mt-2">
                    ...
                    <div class="{{ count($articleLinks) > 0 ? 'col-lg-6' : 'col-lg-9' }} pt-0 pt-sm-2 mt-3">
                        ...
                        <div data-bs-spy="scroll" data-bs-target="#simple-list-example" data-bs-offset="0" data-bs-smooth-scroll="true" class="scrollspy-blog-details" tabindex="0">
                            <img class="max-height-420px w-100 aspect-2 rounded-10 mb-4 mb-sm-5" src="{{ getStorageImages(path: $blogData?->thumbnail_full_url, type:'wide-banner') }}" alt="{{ $product['name'] ?? 'Product' }}">

                            <div class="rich-editor-html-content">
                                {!! $updatedDescription !!}
                            </div>

                            {{-- Product Recommendations — converts blog traffic to sales --}}
                            @include('theme-views.blogs.partials._blog-product-recommendations')
                            </div>
                            </div>
                            ...
                            <div class="d-flex justify-content-center align-items-center gap-3 mt-2">
                                <a href="javascript:" class="share-on-social-media"
                                   ...
                                   data-social-media-name="facebook.com/sharer/sharer.php?u=">
                                    <img width="30" src="{{ theme_asset(path: 'assets/img/blogs/facebook.png') }}" alt="{{ $product['name'] ?? 'Product' }}">
                                </a>
                                <a href="javascript:" class="share-on-social-media"
                                   data-action="{{ route('frontend.blog.details', ['slug' => $blogData['slug'] ?? null]) }}"
                                   data-social-media-name="twitter.com/intent/tweet?text=">
                                    <img width="30" src="{{ theme_asset(path: 'assets/img/blogs/twitter.png') }}" alt="{{ $product['name'] ?? 'Product' }}">
                                </a>
                                <a href="javascript:" class="share-on-social-media"
                                   data-action="{{ route('frontend.blog.details', ['slug' => $blogData['slug'] ?? null]) }}"
                                   data-social-media-name="linkedin.com/shareArticle?mini=true&url=">
                                    <img width="30" src="{{ theme_asset(path: 'assets/img/blogs/linkedin.png') }}" alt="{{ $product['name'] ?? 'Product' }}">
                                </a>
                                <a href="javascript:" class="share-on-social-media"
                                   data-action="{{ route('frontend.blog.details', ['slug' => $blogData['slug'] ?? null]) }}"
                                   data-social-media-name="api.whatsapp.com/send?text=">
                                    <img width="30" src="{{ theme_asset(path: 'assets/img/blogs/whatsapp.png') }}" alt="{{ $product['name'] ?? 'Product' }}">
                                </a>
                            </div>
                            <hr class="my-4">
