...
@section('title', (request('filter') && request('filter') == 'top-vendors' ? translate('top_Stores') : translate('all_Stores')).' | '.$web_config['company_name'].' 'Uganda')
                ...
                <div class="card-body">
                    ...
                    @if (count($vendorsList) == 0)
                        <div class="w-100 text-center pt-5">
                            <img width="80" class="mb-3" src="{{ theme_asset('assets/img/empty-state/empty-vendor.svg') }}" alt="{{ $product['name'] ?? 'Product' }}">
