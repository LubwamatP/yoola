...
Route::group(['prefix' => 'admin', 'as' => 'admin.', 'middleware' => ['admin', 'actch:admin_panel']], function () {
    ...
    // Electricity Tariffs Management
    Route::group(['prefix' => 'electricity-tariffs', 'as' => 'electricity-tariffs.'], function () {
        Route::controller(\App\Http\Controllers\Admin\ElectricityTariffController::class)->group(function () {
            Route::get('/', 'index')->name('index');
    ...
    Route::group(['prefix' => 'ai-operations', 'as' => 'ai-operations.'], function () {
        Route::controller(\App\Http\Controllers\Admin\AIOperationsController::class)->group(function () {
            // Dashboard
            Route::get('/', 'dashboard')->name('dashboard');
            
            // Conversations (AI Chat)
            Route::get('/conversations', 'conversations')->name('conversations');
            Route::get('/conversations/{id}/messages', 'getConversationMessages')->name('conversations.messages');
            Route::post('/conversations/{id}/takeover', 'takeoverConversation')->name('conversations.takeover');
            Route::post('/conversations/{id}/handback', 'handbackConversation')->name('conversations.handback');
            Route::post('/conversations/{id}/send', 'sendMessage')->name('conversations.send');
            Route::post('/chat', 'chat')->name('chat');
            
            // Notifications
            Route::get('/notifications', 'notifications')->name('notifications');
            Route::post('/notifications/trigger', 'triggerNotification')->name('notifications.trigger');
            
            // Power Calculator Leads
            Route::get('/leads', 'leads')->name('leads');
            Route::post('/leads/{id}/contacted', 'markLeadContacted')->name('leads.contacted');
    ...
    Route::group(['prefix' => 'analytics', 'as' => 'analytics.'], function () {
        Route::controller(\App\Http\Controllers\Admin\AnalyticsDashboardController::class)->group(function () {
            Route::get('/', 'index')->name('index');
            Route::get('/live-viewers', 'getLiveViewers')->name('live-viewers');
            Route::get('/most-viewed', 'getMostViewed')->name('most-viewed');
            Route::get('/views-chart', 'getViewsChart')->name('views-chart');
            Route::get('/search-analytics', 'getSearchAnalytics')->name('search-analytics');
            Route::get('/product/{id}', 'getProductAnalytics')->name('product');
            Route::get('/export', 'exportData')->name('export');
        });
    });

    // Power Calculator Settings
    Route::group(['prefix' => 'power-calculator', 'as' => 'power-calculator.'], function () {
        Route::controller(\App\Http\Controllers\Admin\PowerCalculatorSettingsController::class)->group(function () {
            Route::get('/settings', 'index')->name('settings');
            Route::post('/seed-defaults', 'seedDefaults')->name('seed-defaults');
            
            // Tariffs
            Route::post('/tariffs', 'storeTariff')->name('tariffs.store');
            Route::put('/tariffs/{id}', 'updateTariff')->name('tariffs.update');
            Route::delete('/tariffs/{id}', 'deleteTariff')->name('tariffs.delete');
            ...
            // Appliances
            Route::post('/appliances', 'storeAppliance')->name('appliances.store');
            Route::put('/appliances/{id}', 'updateAppliance')->name('appliances.update');
            Route::delete('/appliances/{id}', 'deleteAppliance')->name('appliances.delete');
        });
    });

    // Programmatic SEO - Price Pages
    Route::group(['prefix' => 'price-pages', 'as' => 'price-pages.'], function () {
        Route::controller(\App\Http\Controllers\Admin\Settings\PricePageController::class)->group(function () {
            Route::get('/', 'index')->name('index');
            Route::get('/create', 'create')->name('create');
            Route::post('/', 'store')->name('store');
            Route::get('/{id}/edit', 'edit')->name('edit');
    ...
    Route::group(['prefix' => 'category-landing', 'as' => 'category-landing.'], function () {
        Route::controller(\App\Http\Controllers\Admin\CategoryLandingController::class)->group(function () {
            Route::get('/', 'index')->name('index');
            Route::get('/{id}/edit', 'edit')->name('edit');
            Route::put('/{id}', 'update')->name('update');
            Route::post('/{id}/toggle', 'toggleLandingPage')->name('toggle');
            Route::get('/{id}/preview', 'preview')->name('preview');
            Route::post('/{id}/duplicate', 'duplicate')->name('duplicate');
            Route::post('/bulk-enable', 'bulkEnable')->name('bulk-enable');
        });
    });
});
