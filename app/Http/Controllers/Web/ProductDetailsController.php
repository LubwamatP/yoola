...
class ProductDetailsController extends Controller
{
    ...
    public function getDefaultTheme(string $slug): View|RedirectResponse
    {
        ...
        // Hormozi Upsells: Get complementary products
        $accessories = collect([]);
        try {
            $accessoryIds = \DB::table('product_accessories')
                ->where('product_id', $product['id'])
                ->pluck('accessory_product_id')
                ->toArray();
            if (!empty($accessoryIds)) {
                $accessories = \App\Models\Product::whereIn('id', $accessoryIds)
                    ->where('status', 1)
                    ->take(4)
                    ->get();
            }
        ...
        return view(VIEW_FILE_NAMES['products_details'], compact('product', 'initialProductConfig', 'initialProductQuantity','initialProductPrice','countWishlist', 'countOrder', 'relatedProducts',
                'dealOfTheDay', 'currentDate', 'overallRating', 'wishlistStatus', 'productReviews', 'rating', 'totalReviews', 'productsForReview', 'moreProductFromSeller', 'accessories', 'decimalPointSettings', 'previewFileInfo', 'productAuthorsInfo', 'productPublishingHouseInfo', 'firstVariationQuantity', 'productDetailsMeta'));
        }
