...
class AnalyticsDashboardController extends Controller
{
    ...
    public function index(): View
    {
        ...
        $activeTimeout = 300; // 5 minutes
        $totalActiveSessions = ActiveSession::active($activeTimeout)->count();
        $activeHumans = ActiveSession::active($activeTimeout)->humans()->count();
        $activeBots = ActiveSession::active($activeTimeout)->bots()->count();

        // Bot breakdown by name
        $botBreakdown = ActiveSession::active($activeTimeout)
            ->bots()
        ...
        // Recent sessions (last hour) for human vs bot comparison
        $lastHour = Carbon::now()->subHour();
        $sessionsLastHour = ActiveSession::where('last_activity', '>=', $lastHour)->count();
        $humansLastHour = ActiveSession::where('last_activity', '>=', $lastHour)->humans()->count();
        $botsLastHour = ActiveSession::where('last_activity', '>=', $lastHour)->bots()->count();
        ...
        // Recent bot activity (for display)
        $recentBots = ActiveSession::bots()
