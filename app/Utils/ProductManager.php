...
class ProductManager
{
    ...
    {
        // YOOLA: Apply typo correction
        $name = self::correctSearchTypos($name);
        $key = explode(' ', $name);;
        $user = Helpers::getCustomerInformation($request);
        ...
        $productListData = Product::active()->with(['rating', 'tags', 'clearanceSale' => function ($query) {
            ...
            ->where(function ($q) use ($key) {
                // YOOLA: AND logic - ALL keywords must match in name or slug
    ...
    public static function getProductListData($request, $productUserID = null, $productAddedBy = null, $type = null): mixed
    {
        ...
        $productListData = Product::active()
            ...
            ->when(($request['data_from'] == 'search' && !empty($request['search'])) || !empty($request['name']) || !empty($request['product_name']), function ($query) use ($request) {
                ...
                $productsIDArray = [];
                $searchProducts = ProductManager::search_products($request, $searchKey, 'all', 200, 1);
    ...
    /**
     * YOOLA: Correct common search typos for Uganda market
     */
    public static function correctSearchTypos($query)
    {
        $typos = [
            "hisence" => "hisense", "samsong" => "samsung", "philips" => "philips",
            "phillips" => "philips", "tosiba" => "toshiba", "frige" => "fridge",
            "frezer" => "freezer", "refridgerator" => "refrigerator",
            "telivision" => "television", "micorwave" => "microwave",
            "wahsing" => "washing", "speker" => "speaker", "gepas" => "geepas",
        ];
        $query = strtolower(trim($query));
        foreach ($typos as $typo => $correction) {
            $query = str_ireplace($typo, $correction, $query);
        }
        return $query;
    }
